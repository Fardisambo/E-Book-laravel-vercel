    ...
    <div class="ui-card rounded-3xl  p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-400">Ubah status permintaan dan catatan petugas.</p>
            </div>
            <a href="{{ route('author.borrows.index') }}" class="text-amber-300 hover:underline">Kembali ke daftar</a>
        </div>
        ...
        <form action="{{ route('author.borrows.update', $borrow->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    ...
                    <select name="status" class="w-full rounded-3xl bg-slate-900/80  px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-400">
                ...
                <div>
                    ...
                    <input type="date" name="due_date" value="{{ old('due_date', $borrow->due_date?->format('Y-m-d')) }}" class="w-full rounded-3xl bg-slate-900/80  px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-400">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Catatan Petugas</label>
                <textarea name="admin_notes" rows="4" class="w-full rounded-3xl bg-slate-900/80  px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-amber-400">{{ old('admin_notes', $borrow->admin_notes) }}</textarea>
            </div>
            ...
            <div class="flex flex-wrap gap-4">
                ...
                <a href="{{ route('author.borrows.index') }}" class="inline-flex items-center justify-center rounded-full  px-6 py-3 text-sm text-gray-300 hover:bg-white/5 transition">Batal</a>
            </div>
        </form>
    </div>
</div>
