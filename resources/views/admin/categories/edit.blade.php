...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="ui-card p-6">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full px-4 py-2 border border-white-300 rounded-lg focus:ring-2 focus:ring-[#FF2D20] focus:border-transparent">{{ old('description', $category->description) }}</textarea>
            </div>
        </div>
